...
!yw  endif
   ...
   if(IO_id.eq.my_id) then
     ...
     subroutine HYDRO_exe(did,ITIME) !bdy
...
if (nlst_rt(did)%GWBASESWCRT        .ne. 0 .or. &
   ...
   if(nlst_rt(did)%channel_only .eq. 0 .and. nlst_rt(did)%channelBucket_only .eq. 0) then
      if(nlst_rt(did)%OVRTSWCRT .ne. 0) then
         call OverlandRouting_drv(did,itime) !bdy
      else
...
use module_rt_data, only: hdbdy_n,hdbdy_num,hdbdy_idx,hdbdy_jdx,hdbdy_head,hdbdy_time,hdbdy_ot,hdbdy_t,hdbdy_glonum !bdy
implicit none
integer ntime, did
...
integer i,j,hdbdy_nn !bdy
integer,allocatable :: hdbdy_id(:) !bdy
real,allocatable :: head_temp(:) !bdy
character*5 out1 !bdy
...
if (nlst_rt(did)%CHANRTSWCRT.eq.1 .or. nlst_rt(did)%CHANRTSWCRT .eq. 2 ) then
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!bdy
  open(30,file='boundary_ij.txt')
  open(32,file='boundary_head.txt')
  write(out1,'(I5.5)') my_id
  open(31,file='OUTPUT/boundary_read_id_'//out1//'.txt')
!  open(33,file='OUTPUT/boundary_read_head_'//out1//'.txt')
  hdbdy_num=0
  hdbdy_glonum=0
  do while(.not. eof(30))
      read(30,*) i,j
      hdbdy_glonum=hdbdy_glonum+1
      if(i>=local_startx_rt .and. i<=local_endx_rt .and. j>=local_starty_rt .and. j<=local_endy_rt) hdbdy_num=hdbdy_num+1
  enddo
  if(hdbdy_glonum>99999) then
      write(*,*) 'too much boundary',hdbdy_glonum
      stop
  endif
  rewind(30)
  hdbdy_time=0
  do while(.not. eof(32))
      read(32,*)
      hdbdy_time=hdbdy_time+1
  enddo
  rewind(32)
  allocate(hdbdy_idx(hdbdy_num),hdbdy_jdx(hdbdy_num),hdbdy_head(hdbdy_num,hdbdy_time),hdbdy_ot(hdbdy_time),hdbdy_id(hdbdy_num),head_temp(hdbdy_glonum))
  hdbdy_n=0
  hdbdy_nn=0
  do while(.not. eof(30))
      read(30,*) i,j
      hdbdy_nn=hdbdy_nn+1
      if(i>=local_startx_rt .and. i<=local_endx_rt .and. j>=local_starty_rt .and. j<=local_endy_rt) then
      hdbdy_n=hdbdy_n+1
      hdbdy_idx(hdbdy_n)=i-local_startx_rt+1
      hdbdy_jdx(hdbdy_n)=j-local_starty_rt+1
      hdbdy_id(hdbdy_n)=hdbdy_nn
      write(31,'(6I5)') hdbdy_n,i,j,hdbdy_idx(hdbdy_n),hdbdy_jdx(hdbdy_n),hdbdy_id(hdbdy_n)
      endif 
  enddo
  close(30)
  close(31)
  do hdbdy_nn=1,hdbdy_time
      read(32,*) hdbdy_ot(hdbdy_nn),(head_temp(hdbdy_n),hdbdy_n=1,hdbdy_glonum)
      do hdbdy_n=1,hdbdy_num
          hdbdy_head(hdbdy_n,hdbdy_nn)=head_temp(hdbdy_id(hdbdy_n))
      enddo
!      write(33,'(f5.1,3000f7.3)') hdbdy_ot(hdbdy_nn),(hdbdy_head(hdbdy_n,hdbdy_nn),hdbdy_n=1,hdbdy_num）
  enddo
  close(32)
!  close(33)
  hdbdy_t=1
