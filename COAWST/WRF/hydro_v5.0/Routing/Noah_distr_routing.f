...
!DJG ------------------------------------------------
	...
	SUBROUTINE OV_RTNG(DT,DTRT_TER,IXRT,JXRT,INFXSUBRT,      &
          ...
          SO8RT,SO8RT_D,rt_option,q_sfcflx_x,q_sfcflx_y,ELRT,sea_id,itime) !bdy and bdy for seamask

!yyww 
        use module_mpp_land, only: left_id,down_id,right_id, &
              up_id,mpp_land_com_real, my_id, &
             mpp_land_sync,write_IO_RT_real,write_IO_RT_int,io_id,global_rt_nx, global_rt_ny !bdy and bdy for seamask

        use module_rt_data, only:rt_domain,hdbdy_n,hdbdy_num,hdbdy_idx,hdbdy_jdx,hdbdy_head,hdbdy_time,hdbdy_ot,hdbdy_t,hdbdy_glonum !bdy
        use netcdf !bdy
...
!!!!!!!!!!!!!!!!!!!!!!!bdy
        ...
        integer, intent(inout), dimension(ixrt,jxrt) :: sea_id !for seamask 
        integer :: itime
        character*4 out1,out2
        integer :: iret,ncid,varid
        integer,parameter :: NF_CLOBBER=0, NF_64BIT_OFFSET=512, nf_unlimited=0, nf_float=5, nf_int=4
        integer :: dimid_ix, dimid_jx, dimid_times
        real xdumd(global_rt_nx,global_rt_ny)
        integer ydumd(global_rt_nx,global_rt_ny) !for seamask
        integer out_interval,out_count,out_n
        real SFCHEADSUBRT_tmp(ixrt,jxrt) !for seamask
...
!DJG Assign all infiltration excess to surface head...
            ...
            SFCHEADSUBRT_tmp=SFCHEADSUBRT !bdy for seamask
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!bdy
      out_interval=45
      out_count=DT_FRAC/out_interval
      if(my_id==io_id) then          
        write(out1,'(I4.4)') itime
        iret = nf_create('OUTPUT/'//out1//'.nc', IOR(NF_CLOBBER,NF_64BIT_OFFSET), ncid)
        iret = nf_def_dim(ncid, "time", out_count, dimid_times)
        iret = nf_def_dim(ncid, "x", global_rt_nx, dimid_ix)
        iret = nf_def_dim(ncid, "y", global_rt_ny, dimid_jx)  
        ...
        iret = nf_def_var(ncid,"sea_id",NF_INT, 3, (/dimid_ix,dimid_jx,dimid_times/), varid) !for seamask
        iret = nf_put_att_text(ncid,varid,'id=1',3,'sea') !for seamask
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!bdy
   if(itime>hdbdy_ot(hdbdy_t)) then
       hdbdy_t=hdbdy_t+1
   endif
   if(hdbdy_t<=hdbdy_time) then
       call add_HDBDY(IXRT,JXRT,SFCHEADSUBRT)
   endif
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!bdy
      if(mod(KRT,out_interval)==1) then
        call write_IO_rt_real(SFCHEADSUBRT,xdumd) 
        call write_IO_rt_int(sea_id,ydumd) !for seamask
      if(my_id==io_id) then
        out_n=KRT/out_interval+1
        iret = nf_inq_varid(ncid,  "sfcheadsubrt", varid)
        iret = nf_put_vara_real(ncid, varid, (/1,1,out_n/), (/global_rt_nx,global_rt_ny,1/), xdumd)
        
        iret = nf_inq_varid(ncid,  "sea_id", varid) !for seamask
        iret = nf_put_vara_int(ncid, varid, (/1,1,out_n/), (/global_rt_nx,global_rt_ny,1/), ydumd) !for seamask 
      endif
      endif
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! AD hardwire to force channel retention depth to be 5mm.
             ...
             IF (SFCHEADSUBRT(I,J).GT.RETDEPRT(I,J)) THEN
              ...
              if(sea_id(i,j)==0) then !bdy for seamask
...
!!               QINFLO(CH_NET(I,J)=QINFLO(CH_NET(I,J)+SFCHEAD(I,J) - RETDEPRT(I,J)
             ...
             !  endif
               ...
               QSTRMVOLTRT = QSTRMVOLTRT + (SFCHEADSUBRT(I,J) - SFCHEADSUBRT_tmp(I,J))
               QSTRMVOLRT(I,J) = QSTRMVOLRT(I,J)+SFCHEADSUBRT(I,J)-SFCHEADSUBRT_tmp(I,J)
...
!DJG Debug...
		...
		SOYRT,RETDEPRT,OVROUGHRT,IXRT,JXRT,QBDRYRT,QBDRYTRT,    & 
                SO8RT,SO8RT_D,q_sfcflx_x,q_sfcflx_y,ELRT,sea_id,itime) !bdy and bdy for seamask
            else
...
!DJG ----------------------------------------------------------------
     ...
     &     q_sfcflx_x,q_sfcflx_y,ELRT,sea_id,itime) !bdy and bdy for seamask
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        use module_mpp_land, only: left_id,down_id,right_id, &
              up_id,mpp_land_com_real,mpp_land_com_integer, my_id, mpp_land_com_real8,&
             mpp_land_sync !bdy for seamask
...
!! Declare passed arrays
        ...
        REAL, INTENT(IN), DIMENSION(XX,YY) :: ELRT !bdy
        integer, intent(INOUT), dimension(XX,YY) :: sea_id !bdy for seamask
!!! Declare Local Variables
...
!!!!!!!!!!!!!!!!!!!!!!!!!!bdy               
              call GETMAX8DIR(IXX0,JYY0,I,J,H,RETENT_DEP,so8rt,gsize(i,j,:),sfx,XX,YY,ELRT)
!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!yw changed as following:
      ...
      if(sea_id(i,j)==1 .and. sea_id(ixx0,jyy0)==0) sea_id(ixx0,jyy0)=1 !bdy for seamask
...
!!!!!!!!!!!!!bdy for seamask
        do j=2,YY-1
          do i=2,XX-1
              if(H(i,j)<=100 .and. ELRT(i,j)>=0 .and. sea_id(i,j)==1) then
                sea_id(i,j)=0
              endif
          enddo
        enddo
!!!!!!!!!!!!!
        ...
        call MPP_LAND_COM_INTEGER(sea_id,XX,YY,99) !bdy for seamask
...
!DJG ----------------------------------------------------------------
        SUBROUTINE GETMAX8DIR(IXX0,JYY0,I,J,H,RETENT_DEP,sox,tmp_gsize,max,XX,YY,ELRT) !bdy

        use module_rt_data, only: hdbdy_n,hdbdy_num,hdbdy_idx,hdbdy_jdx,hdbdy_head !bdy        
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!bdy          
        ...
        ind=0  
        do hdbdy_n=1,hdbdy_num
            if(i==hdbdy_idx(hdbdy_n) .and. j==hdbdy_jdx(hdbdy_n)) ind=1
        enddo          
...
! This subroutine returns: ZWATTABLRT, CWATAVAIL and SATLYRCHK
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!bdy
    subroutine add_HDBDY_drv(did)
        use module_RT_data, only: rt_domain
        use module_namelist, only: nlst_rt    
        implicit none
        integer :: did
        call add_HDBDY(rt_domain(did)%ixrt,rt_domain(did)%jxrt,rt_domain(did)%INFXSUBRT)
    end subroutine add_HDBDY_drv
    
    subroutine add_HDBDY(ixrt,jxrt,INFXSUBRT)
        use module_rt_data, only: hdbdy_n,hdbdy_num,hdbdy_idx,hdbdy_jdx,hdbdy_head,hdbdy_time,hdbdy_ot,hdbdy_t,hdbdy_glonum
        implicit none
        integer, INTENT(IN) :: ixrt, jxrt
        REAL, INTENT(OUT), DIMENSION(IXRT,JXRT)   :: INFXSUBRT
        
        do hdbdy_n=1,hdbdy_num
            INFXSUBRT(hdbdy_idx(hdbdy_n),hdbdy_jdx(hdbdy_n))=hdbdy_head(hdbdy_n,hdbdy_t)*1000
        enddo
        
    end subroutine add_HDBDY
    !!!!!!!!!!!!!!!!!!!!!!!!!!!
   
       subroutine OverlandRouting_drv(did,itime) !bdy
             use module_RT_data, only: rt_domain
             use module_namelist, only: nlst_rt
             implicit none
             integer :: did,itime !bdy
             if(nlst_rt(did)%OVRTSWCRT .eq. 1) then
                 call OverlandRouting (nlst_rt(did)%DT, nlst_rt(did)%DTRT_TER, nlst_rt(did)%rt_option, &
                          ...
                          rt_domain(did)%smctot1,rt_domain(did)%dsmctot,RT_DOMAIN(did)%ELRT,RT_DOMAIN(did)%sea_id,itime ) !bdy and bdy for seamask
...
!           zeroing out just in case
       ...
       subroutine OverlandRouting (DT, DTRT_TER, rt_option, ixrt, jxrt,LAKE_MSKRT, &
                  ...
                  dist, SO8RT, SO8RT_D, &
                  SMCTOT2,suminfxs1,suminfxsrt,smctot1,dsmctot,ELRT,sea_id, itime ) !bdy and bdy for seamask
...
! local variable
       ...
       real            :: qbdry1,qbdry2
       integer :: sfcrt_flag
       REAL, INTENT(IN), DIMENSION(IXRT,JXRT)   :: ELRT !bdy
       integer, intent(inout), dimension(ixrt,jxrt) :: sea_id !bdy for seamask
       integer :: itime !bdy
