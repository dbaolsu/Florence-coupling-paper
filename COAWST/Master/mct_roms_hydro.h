...
**                                                                    **
************************************************************************
** Modified by Coupled Ocean Modeling Group at LSU       Daoyang Bao  **
** in 2022 for ROMS-WRF-Hydro coupling 
************************************************************************
*/
...
!     real(r8), parameter ::  Large = 1.0E+20_r8
      ...
      real(r8) :: WaterLevel(LBi:UBi,LBj:UBj),WaterLevelout(LBi:UBi,LBj:UBj),diff_u,dzdl,Hu,RDRAG2u,uvtmp !bdy for hyd2ocn 
...
!
     ...
     &                    Waterlevel)
      ...
      DO j=LBj,UBj
        DO i=LBi,UBi
          if(Waterlevel(i,j)<9000 .and. Waterlevel(i,j)>-9000) then
              if(Waterlevel(i,j)<=range(1)) then
                range(1)=Waterlevel(i,j)
              endif
              if(Waterlevel(i,j)>=range(2)) then
                range(2)=Waterlevel(i,j)
              endif
          endif
        ENDDO
      ENDDO
...
# endif
      IF (Myrank.eq.MyMaster) THEN
        write(stdout,40) 'HYDtoROMS Min/Max  Waterlevel    (m):  ',     &
     &                    range(1),range(2)
      END IF
      ...
      DO j=LBj,UBj
        DO i=LBi,UBi
           if(Waterlevel(i,j)<9000 .and. Waterlevel(i,j)>-9000) then
               if(i>LBi) then
               if(Waterlevel(i-1,j)>9000) then
                  Nsrc(ng)=Nsrc(ng)+1
                  SOURCES(ng)%Dsrc(Nsrc(ng))=0.0_r8
                  SOURCES(ng)%Isrc(Nsrc(ng))=i
                  SOURCES(ng)%Jsrc(Nsrc(ng))=j
                  if( ((WaterLevel(i,j)+GRID(ng)%h(i,j)>0.15) .and. ((OCEAN(ng)%zeta(i-1,j,knew(ng))+GRID(ng)%h(i-1,j)>0.15) .or. (OCEAN(ng)%zeta(i-1,j,knew(ng))<Waterlevel(i,j)))) &
     &           .or. ((OCEAN(ng)%zeta(i-1,j,knew(ng))+GRID(ng)%h(i-1,j)>0.15) .and. ((WaterLevel(i,j)+GRID(ng)%h(i,j)>0.15) .or. (OCEAN(ng)%zeta(i-1,j,knew(ng))>Waterlevel(i,j)))) ) then
                    
                    dzdl=(OCEAN(ng)%zeta(i-1,j,knew(ng))-Waterlevel(i,j))*GRID(ng)%pm(i,j)
                    Hu=0.5*(GRID(ng)%h(i,j)+GRID(ng)%h(i-1,j)+OCEAN(ng)%zeta(i-1,j,knew(ng))+Waterlevel(i,j))
                    RDRAG2u=0.5*(GRID(ng)%rdrag2(i,j)+GRID(ng)%rdrag2(i-1,j))
                    if(dzdl==0) then
                        diff_u=0
                    else
                        diff_u=dzdl/abs(dzdl)*sqrt(9.81*abs(dzdl)*Hu/RDRAG2u)
                    endif
                    
                    uvtmp=SOURCES(ng)%Qbar(Nsrc(ng))+9.81*dzdl*TI_OCN2HYD-SOURCES(ng)%Qbar(Nsrc(ng))*abs(SOURCES(ng)%Qbar(Nsrc(ng)))*RDRAG2u/Hu*TI_OCN2HYD

                    if(isnan(uvtmp) .or. uvtmp>=HUGE(uvtmp) .or. uvtmp<=-HUGE(uvtmp)) then
                        SOURCES(ng)%Qbar(Nsrc(ng))=diff_u
                    elseif(uvtmp>=SOURCES(ng)%Qbar(Nsrc(ng))) then
                        if(uvtmp>diff_u) then
                            SOURCES(ng)%Qbar(Nsrc(ng))=diff_u
                        else
                            SOURCES(ng)%Qbar(Nsrc(ng))=uvtmp
                        endif
                    elseif(uvtmp<SOURCES(ng)%Qbar(Nsrc(ng))) then
                        if(uvtmp<diff_u) then
                            SOURCES(ng)%Qbar(Nsrc(ng))=diff_u
                    ...
                    cff=(OCEAN(ng)%zeta(i-1,j,knew(ng))-Waterlevel(i,j))/(GRID(ng)%pm(i,j)*GRID(ng)%pn(i,j)*GRID(ng)%pm(i-1,j)*GRID(ng)%pn(i-1,j))/(1/(GRID(ng)%pm(i,j)*GRID(ng)%pn(i,j))+1/(GRID(ng)%pm(i-1,j)*GRID(ng)%pn(i-1,j)))
                    if(OCEAN(ng)%zeta(i-1,j,knew(ng))>Waterlevel(i,j)) then  
                      if(SOURCES(ng)%Qbar(Nsrc(ng))*0.5_r8*GRID(ng)%on_u(i,j)*(GRID(ng)%h(i,j)+GRID(ng)%h(i-1,j)+OCEAN(ng)%zeta(i-1,j,knew(ng))+Waterlevel(i,j))*TI_OCN2HYD>cff) then
                        SOURCES(ng)%Qbar(Nsrc(ng))=cff/(0.5_r8*GRID(ng)%on_u(i,j)*(GRID(ng)%h(i,j)+GRID(ng)%h(i-1,j)+OCEAN(ng)%zeta(i-1,j,knew(ng))+Waterlevel(i,j)))/TI_OCN2HYD
                      endif
                    elseif(OCEAN(ng)%zeta(i-1,j,knew(ng))<Waterlevel(i,j)) then
                      if(SOURCES(ng)%Qbar(Nsrc(ng))*0.5_r8*GRID(ng)%on_u(i,j)*(GRID(ng)%h(i,j)+GRID(ng)%h(i-1,j)+OCEAN(ng)%zeta(i-1,j,knew(ng))+Waterlevel(i,j))*TI_OCN2HYD<cff) then
                        SOURCES(ng)%Qbar(Nsrc(ng))=cff/(0.5_r8*GRID(ng)%on_u(i,j)*(GRID(ng)%h(i,j)+GRID(ng)%h(i-1,j)+OCEAN(ng)%zeta(i-1,j,knew(ng))+Waterlevel(i,j)))/TI_OCN2HYD
                      endif
                    endif

                    if(SOURCES(ng)%Qbar(Nsrc(ng))>0) then
                      cff=(OCEAN(ng)%zeta(i-1,j,knew(ng))+GRID(ng)%h(i-1,j))/(GRID(ng)%pm(i-1,j)*GRID(ng)%pn(i-1,j))
                      if(SOURCES(ng)%Qbar(Nsrc(ng))*0.5_r8*GRID(ng)%on_u(i,j)*(GRID(ng)%h(i,j)+GRID(ng)%h(i-1,j)+OCEAN(ng)%zeta(i-1,j,knew(ng))+Waterlevel(i,j))*TI_OCN2HYD>cff) then
                        SOURCES(ng)%Qbar(Nsrc(ng))=cff/(0.5_r8*GRID(ng)%on_u(i,j)*(GRID(ng)%h(i,j)+GRID(ng)%h(i-1,j)+OCEAN(ng)%zeta(i-1,j,knew(ng))+Waterlevel(i,j)))/TI_OCN2HYD
                      endif
                    elseif(SOURCES(ng)%Qbar(Nsrc(ng))<0) then
                      cff=-(Waterlevel(i,j)+GRID(ng)%h(i,j))/(GRID(ng)%pm(i,j)*GRID(ng)%pn(i,j))
                      if(SOURCES(ng)%Qbar(Nsrc(ng))*0.5_r8*GRID(ng)%on_u(i,j)*(GRID(ng)%h(i,j)+GRID(ng)%h(i-1,j)+OCEAN(ng)%zeta(i-1,j,knew(ng))+Waterlevel(i,j))*TI_OCN2HYD<cff) then
                        SOURCES(ng)%Qbar(Nsrc(ng))=cff/(0.5_r8*GRID(ng)%on_u(i,j)*(GRID(ng)%h(i,j)+GRID(ng)%h(i-1,j)+OCEAN(ng)%zeta(i-1,j,knew(ng))+Waterlevel(i,j)))/TI_OCN2HYD
                      endif
                    endif
                      ...
                      SOURCES(ng)%Qbar(Nsrc(ng))=0

                  endif

                  range(1)=MIN(range(1),SOURCES(ng)%Qbar(Nsrc(ng)))
                  range(2)=MAX(range(2),SOURCES(ng)%Qbar(Nsrc(ng)))
     ...
     &           .or. ((OCEAN(ng)%zeta(i+1,j,knew(ng))+GRID(ng)%h(i+1,j)>0.15) .and. ((WaterLevel(i,j)+GRID(ng)%h(i,j)>0.15) .or. (OCEAN(ng)%zeta(i+1,j,knew(ng))>Waterlevel(i,j)))) ) then
               ...
               if(j>LBj) then
               if(Waterlevel(i,j-1)>9000) then
                  Nsrc(ng)=Nsrc(ng)+1
                  SOURCES(ng)%Dsrc(Nsrc(ng))=1.0_r8
                  SOURCES(ng)%Isrc(Nsrc(ng))=i
                  SOURCES(ng)%Jsrc(Nsrc(ng))=j
                  if( ((WaterLevel(i,j)+GRID(ng)%h(i,j)>0.15) .and. ((OCEAN(ng)%zeta(i,j-1,knew(ng))+GRID(ng)%h(i,j-1)>0.15) .or. (OCEAN(ng)%zeta(i,j-1,knew(ng))<Waterlevel(i,j)))) &
     &           .or. ((OCEAN(ng)%zeta(i,j-1,knew(ng))+GRID(ng)%h(i,j-1)>0.15) .and. ((WaterLevel(i,j)+GRID(ng)%h(i,j)>0.15) .or. (OCEAN(ng)%zeta(i,j-1,knew(ng))>Waterlevel(i,j)))) ) then

                    dzdl=(OCEAN(ng)%zeta(i,j-1,knew(ng))-Waterlevel(i,j))*GRID(ng)%pm(i,j)
                    Hu=0.5*(GRID(ng)%h(i,j)+GRID(ng)%h(i,j-1)+OCEAN(ng)%zeta(i,j-1,knew(ng))+Waterlevel(i,j))
                    RDRAG2u=0.5*(GRID(ng)%rdrag2(i,j)+GRID(ng)%rdrag2(i,j-1))
                    if(dzdl==0) then
                        diff_u=0
                    else
                        diff_u=dzdl/abs(dzdl)*sqrt(9.81*abs(dzdl)*Hu/RDRAG2u)
                    ...
                    cff=(OCEAN(ng)%zeta(i,j-1,knew(ng))-Waterlevel(i,j))/(GRID(ng)%pm(i,j)*GRID(ng)%pn(i,j)*GRID(ng)%pm(i,j-1)*GRID(ng)%pn(i,j-1))/(1/(GRID(ng)%pm(i,j)*GRID(ng)%pn(i,j))+1/(GRID(ng)%pm(i,j-1)*GRID(ng)%pn(i,j-1)))
                    if(OCEAN(ng)%zeta(i,j-1,knew(ng))>Waterlevel(i,j)) then
                      if(SOURCES(ng)%Qbar(Nsrc(ng))*0.5_r8*GRID(ng)%on_u(i,j)*(GRID(ng)%h(i,j)+GRID(ng)%h(i,j-1)+OCEAN(ng)%zeta(i,j-1,knew(ng))+Waterlevel(i,j))*TI_OCN2HYD>cff) then
                        SOURCES(ng)%Qbar(Nsrc(ng))=cff/(0.5_r8*GRID(ng)%on_u(i,j)*(GRID(ng)%h(i,j)+GRID(ng)%h(i,j-1)+OCEAN(ng)%zeta(i,j-1,knew(ng))+Waterlevel(i,j)))/TI_OCN2HYD
                      endif
                    elseif(OCEAN(ng)%zeta(i,j-1,knew(ng))<Waterlevel(i,j)) then
                      if(SOURCES(ng)%Qbar(Nsrc(ng))*0.5_r8*GRID(ng)%on_u(i,j)*(GRID(ng)%h(i,j)+GRID(ng)%h(i,j-1)+OCEAN(ng)%zeta(i,j-1,knew(ng))+Waterlevel(i,j))*TI_OCN2HYD<cff) then
                        SOURCES(ng)%Qbar(Nsrc(ng))=cff/(0.5_r8*GRID(ng)%on_u(i,j)*(GRID(ng)%h(i,j)+GRID(ng)%h(i,j-1)+OCEAN(ng)%zeta(i,j-1,knew(ng))+Waterlevel(i,j)))/TI_OCN2HYD
                      endif
                    endif
                    ...
                    if(SOURCES(ng)%Qbar(Nsrc(ng))>0) then
                      cff=(OCEAN(ng)%zeta(i,j-1,knew(ng))+GRID(ng)%h(i,j-1))/(GRID(ng)%pm(i,j-1)*GRID(ng)%pn(i,j-1))
                      if(SOURCES(ng)%Qbar(Nsrc(ng))*0.5_r8*GRID(ng)%on_u(i,j)*(GRID(ng)%h(i,j)+GRID(ng)%h(i,j-1)+OCEAN(ng)%zeta(i,j-1,knew(ng))+Waterlevel(i,j))*TI_OCN2HYD>cff) then
                        SOURCES(ng)%Qbar(Nsrc(ng))=cff/(0.5_r8*GRID(ng)%on_u(i,j)*(GRID(ng)%h(i,j)+GRID(ng)%h(i,j-1)+OCEAN(ng)%zeta(i,j-1,knew(ng))+Waterlevel(i,j)))/TI_OCN2HYD
                      endif
                    elseif(SOURCES(ng)%Qbar(Nsrc(ng))<0) then
...
# endif
      IF (Myrank.eq.MyMaster) THEN
        write(stdout,40) 'HYDtoROMS MaxQbar +ROMS2HYD -HYD2ROMS (m/s):  ',     &
     &                    range(1),range(2)
      END IF
